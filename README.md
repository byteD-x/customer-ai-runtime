...
- **低成本治理** - 文本链路记录 provider、model、route、token、usage 来源、币种、账期、可配置模型价格估算、缓存命中与预算告警；支持通过 `default_model` / `route_model_map` 做轻量模型路由，按 route / tenant / default 选择 `selected_model`，但不自动切换 provider；支持导入 provider billing 样本并在管理端按 provider / route / model 区分本地估算成本、provider 账单样本金额、诊断样本差异和 usage token 对账摘要；导入响应会返回非阻断的 `quality_issue_count` / `quality_issues`，用于提示本地样本质量问题；知识问答安全缓存，业务查询保持实时不缓存，并输出单实例缓存运行时统计
...
powershell -ExecutionPolicy Bypass -File scripts\interview-demo.ps1 -Markdown -OutputPath .codex\interview-demo-report.md
# 可选：使用脚本封装演示命令
...
上述 `.codex\*-report.*` 文件是本地演示和审计生成物，已在 `.gitignore` 中排除，便于反复生成而不污染提交。

当前本地实测基线以本节命令输出为准；演示输出包含 `route`、`model_route`、`citations`、`finance_knowledge`、`saas_knowledge`、`tool_result`、`handoff_package`、`handoff_queue`、`claimed_session`、`agent_workflow`、`cost_summary`、`rag_eval_summary`、`rag_quality_gate`，便于在面试中现场说明“低成本、高效率、可治理”的 AI 客服链路。`model_route` 展示 `route_map` / `tenant_map` / `default_model` / `static_route` 的模型选择策略，说明项目可以用 `default_model` 和 `route_model_map` 做轻量模型路由，但不会自动切换 provider。`rag_quality_gate` 会把 RAG 评测汇总成是否通过、失败 case、badcase breakdown 和修复建议，方便面试时直接讲“能复盘、能回归”。`powershell -ExecutionPolicy Bypass -File scripts\interview-demo.ps1 -Markdown -OutputPath .codex\interview-demo-report.md` 可以直接生成一页式 Markdown 报告文件，适合贴给面试官或放进作品集。`finance_knowledge` 展示财务运营知识库样例的 `finance operations policy` 引用，覆盖报销审批所需发票和成本中心等行业问答证据；`saas_knowledge` 展示 SaaS 管理知识库样例的 `saas administration policy` 引用，覆盖 SCIM 同步 15 分钟等企业应用支持口径；`agent_workflow` 展示受控工具白名单、最大步骤数、顺序执行计划和逐步 trace，覆盖“订单状态 -> 物流轨迹”的 Agent 工具流面试讲点。`cost_summary` 会区分 `estimated_cost_cents`（本地模型价格表估算）与 `provider_billed_cost_cents`（导入的 provider billing 样本金额），通过 `cost_reconciliation` 展示诊断样本口径的 `variance_cents = provider_billed_cost_cents - estimated_cost_cents`，并通过 `by_model` 展示不同模型的请求数、token、估算成本和导入账单样本金额，通过 `usage_reconciliation` 按 `tenant_id + session_id + provider + model` 对运行时 usage 与导入 provider billing usage 做 token 对账诊断，区分强匹配、未匹配和弱归因样本；provider billing 导入响应中的 `quality_issue_count` / `quality_issues` 只提示本地样本质量问题，不阻断导入。RAG eval 中的 `context_precision` / `context_recall` 是基于本地标注关键词和返回引用文本的启发式离线指标，用于暴露额外无关引用或上下文遗漏，不代表线上真实检索精度。`check_external_readiness.py` 会检查 OpenAI models、OpenAI Admin usage/costs、Qdrant runtime config/health/collections、业务 API、客服工单 API、Redis/Postgres 队列依赖的配置、配置一致性与可达性，并在顶层 `audit` 返回 `scope`、`generated_at`、`timeout_seconds`、`evidence_level` 和免责声明，在每个检查项的 `audit` 返回 `category`、`probe_type`、`required_env`、`optional_env`、`evidence`，用于审计检查口径且不暴露真实密钥；未配置或未启用对应 provider 时返回 `skipped`。只有真实凭据、网络和外部系统可达时才能声明对应联调通过。`eval_online_rag.py` 只基于你提供的脱敏线上标注样本计算 `online_accuracy`，没有样本时不代表线上准确率。上述结果只代表当前本地样例、导入样本或输入样本，不代表真实成本节省、自动 provider 账单拉取、外部 provider 端到端联调结果或生产 SLA。
