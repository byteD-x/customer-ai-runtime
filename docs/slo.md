...
- 离线 RAG eval：`scripts/eval_rag.py` 输出 `pass_rate`、`effective_hit_rate`、`offline_accuracy`、`reviewed_case_count`、`cohort_breakdown` 和失败明细
...
- `rag_eval_pass_rate` / `offline_accuracy`：仅用于本地标注 case 回归；当前样例包含标注集元数据、灰度 cohort 和人工复核状态，但线上口径仍必须基于真实业务标注集、灰度流量和人工复核
...
- 将 RAG eval 从本地标注样例扩展为真实业务标注集、线上灰度流量和反馈闭环。
