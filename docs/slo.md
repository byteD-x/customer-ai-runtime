...
- 离线 RAG eval：`scripts/eval_rag.py` 输出 `pass_rate`、`effective_hit_rate`、`citation_accuracy`、`refusal_accuracy`、`faithfulness_score`、`offline_accuracy`、`reviewed_case_count`、`cohort_breakdown` 和失败明细
...
- `rag_eval_pass_rate` / `offline_accuracy`：仅用于本地标注 case 回归；当前样例包含标注集元数据、灰度 cohort、人工复核状态、引用准确率、拒答准确率和启发式 faithfulness 分数，但线上口径仍必须基于真实业务标注集、灰度流量和人工复核
