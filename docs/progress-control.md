...
- 面试项目强化能力已落地：成本治理与知识问答缓存、可配置模型价格表、RAG eval、多知识库样例、结构化交接包、人工接管队列、面试演示脚本和 STAR 材料。
...
| 8 | 基础骨架 | 已完成 | 基础工程已存在 |
| 9-16 | 实现与联调阶段 | 已完成当前面试基线 | 成本治理、RAG eval、接管队列、演示脚本和测试已补齐 |
...
- 行业适配器、知识域管理与上下文解析接口
- 插件管理接口与多认证模式测试
- LLM usage / cache hit / estimated cost 记录与成本摘要接口
- 可配置模型价格表，用于按 provider / model 估算本轮调用成本
- 知识问答安全缓存与业务查询不缓存策略
- RAG eval 8 个本地 cases、多知识库样例、评测脚本与失败明细
- 结构化 `handoff_package`：情绪、问题摘要、最后用户消息、相关业务对象、页面上下文与行为信号
- 人工接管队列、技能组、优先级排序和 `claim-next`
- 面试演示脚本与 STAR/简历材料补充
...
- 当前 RAG eval 是本地小样本 case，不代表线上准确率。
- 当前成本支持本地模型价格表估算；真实账单结算仍需要接入 provider 原生 usage、租户预算、币种和结算周期。
- 当前人工接管队列适合单实例；多实例原子认领是 future target。
