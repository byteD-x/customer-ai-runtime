...
- 管理端聚合：`GET /api/v1/admin/costs/summary`，按 provider / route 汇总 token、估算成本、导入账单样本金额、诊断样本成本差异、provider billing usage 对账诊断、缓存命中、provider usage 记录数、provider billing 样本数、usage 来源、cost 来源、币种和账期；`GET /api/v1/admin/metrics/summary` 输出单实例 `response_cache_summary`

### 设计取舍

- 本地 provider 默认估算 token，并按本地模型价格表估算成本，用于演示治理链路；预算阈值、币种和账期支持全局策略与租户级覆盖；provider billing 样本导入只用于把外部账单样本纳入摘要观察，不与本地估算成本混算。导入响应中的 `quality_issues` 只提示 usage 时间窗、归因维度、外部记录 ID、币种和账期等本地样本质量问题，不阻断导入。成本摘要中的 `cost_reconciliation` 和 bucket 级 `cost_variance_cents` 只表示导入样本金额减去本地估算金额的诊断差异；`usage_reconciliation` 只按 `tenant_id + session_id + provider + model` 对运行时 usage 与导入 provider billing usage 做 token 对账诊断；当估算成本为 0 时，`cost_variance_ratio` 返回 `null`。自动拉取 provider 真实账单和完整结算系统仍属于 future target。
...
- 当前成本治理包含本地模型价格表估算、provider billing 样本导入、非阻断样本质量诊断、诊断样本成本差异摘要与 `usage_reconciliation` token 对账摘要，并显式暴露 usage 来源、cost 来源、可配置币种、可配置账期和可配置本地预算阈值；导入样本不代表自动真实账单同步、完整租户结算或线上节省比例。
