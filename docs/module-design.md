...
- 管理端聚合：`GET /api/v1/admin/costs/summary`，按 provider / route 汇总 token、估算成本、导入账单样本金额、缓存命中、provider usage 记录数、provider billing 样本数、usage 来源、cost 来源、币种和账期；`GET /api/v1/admin/metrics/summary` 输出单实例 `response_cache_summary`

### 设计取舍

- 本地 provider 默认估算 token，并按本地模型价格表估算成本，用于演示治理链路；预算阈值、币种和账期支持全局策略与租户级覆盖；provider billing 样本导入只用于把外部账单样本纳入摘要观察，不与本地估算成本混算。自动拉取 provider 真实账单和完整结算系统仍属于 future target。
...
- 当前成本治理包含本地模型价格表估算与 provider billing 样本导入，并显式暴露 usage 来源、cost 来源、可配置币种、可配置账期和可配置本地预算阈值；导入样本不代表自动真实账单同步、完整租户结算或线上节省比例。
