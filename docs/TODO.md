...
- 成本治理：LLM usage、usage 来源、cost 来源、可配置币种、可配置账期、可配置本地预算阈值、租户成本策略、可配置模型价格表、provider billing 样本导入、知识问答安全缓存、业务查询不缓存、成本摘要
...
- **真实成本结算**：当前已完成租户级预算阈值、币种和账期策略配置，以及 provider billing 样本导入与摘要聚合；后续继续接入自动 provider 账单拉取、真实 provider usage 对账与账单结算系统。
