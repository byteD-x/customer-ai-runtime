...
15. 面试材料包：`powershell -ExecutionPolicy Bypass -File scripts\interview-package.ps1` 或 Python 跨平台入口 `.venv\Scripts\python.exe scripts\interview_package.py` 可以一次生成 demo、RAG eval 和 readiness 报告；Linux/macOS 激活虚拟环境后可将 `.venv\Scripts\python.exe` 替换为 `python`；如有脱敏线上样本，再加 `-OnlineRagSamplePath examples\online_rag_sample.jsonl` 或 `--online-rag-sample-path examples\online_rag_sample.jsonl` 生成 online eval 报告。
