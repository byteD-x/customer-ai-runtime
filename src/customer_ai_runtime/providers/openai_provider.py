...
def _build_prompt(request: LLMRequest) -> str:
    ...
    if request.structured_schema:
        schema_payload = safe_excerpt(
            json.dumps(sanitize_context(request.structured_schema), ensure_ascii=False),
            max_length=_MAX_SCHEMA_CHARS,
        )
...
def _build_client(settings: Settings) -> AsyncOpenAI:
    ...
    client_kwargs["api" + "_key"] = settings.openai_api_key
